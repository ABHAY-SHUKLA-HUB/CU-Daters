...
// ===== SIGNUP (After OTP Verification - OTP system removed) =====
router.post('/signup', asyncHandler(async (req, res, _next) => {
  console.log('\n========== SIGNUP REQUEST (Complete Profile) ==========');
  
  const {
    // Basic info (for initial user creation, now without OTP)
    name,
    email,
    phone,
    password,
  ...
  // If user doesn't exist, create it now (OTP system removed - no verification required)
  if (!user) {
    if (!name || !phone || !password || !college) {
      throw new AppError('Initial registration data missing: name, email, phone, password, and college are required', 400);
    }
    ...
    user = new User({
      ...
      role: 'user',
      subscription_status: 'none',
      emailVerified: true // OTP removed - marking as verified immediately
    });
  }
